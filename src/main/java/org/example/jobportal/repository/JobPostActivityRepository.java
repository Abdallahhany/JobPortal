...
public interface JobPostActivityRepository extends JpaRepository<JobPostActivity, Integer> {
    // Query to get the list of jobs posted by a recruiter along with the total number of candidates for each job
    ...
    // Query to search for job posts based on job title, location, job type, and remote status without considering the posted date
    ...
    // Query to search for job posts based on job title, location, job type, remote status, and posted date
    @Query(value = "SELECT * FROM job_post_activity j INNER JOIN job_location l on j.job_location_id=l.id  WHERE j" +
            ".job_title LIKE %:job%"
            + " AND (l.city LIKE %:location%"
            + " OR l.country LIKE %:location%"
